...
def instance_segmentation(
    ...
    min_area: int = 100,
    nms_iou: float = 0.6,            # only suppress STRONG duplicates (Mask2Former uses no NMS)
    containment: float = 1.0,        # 1.0 = OFF: keep heavily-occluded instances; never drop
                                     # a mask just because it sits inside a larger one
    resolve_overlaps: bool = True,   # per-pixel argmax still resolves the actual overlap
    ...
    Key settings for multi-person scenarios:
    - nms_iou=0.2: Removes near-identical duplicate masks
    - containment=0.75: Merges fragments of the same person
    - resolve_overlaps=True: Ensures non-overlapping masks (paper Fig. 7)
    """
    ...
    # ---- NMS: keep highest-scoring; drop near-duplicates AND fragments ----
    # Two suppression rules, both processed high-score -> low-score:
    #   * IoU > nms_iou           : near-identical duplicate of a kept mask.
    #   * inter/area_i > containment: this (lower-score) mask is mostly INSIDE a kept
    #     mask -> it is a *fragment* of that same person (the phase-2 "several masks
    #     on one person" symptom). Adjacent people touch only at the border, so their
    #     containment stays low and both are kept (recall preserved).
    order = torch.argsort(cls_score, descending=True).tolist()
    kept: List[int] = []
    for i in order:
        area_i = int(bin_mask[i].sum())
        if area_i < min_area:
            continue
        duplicate = False
        for j in kept:
            ...
            if (inter / union) > nms_iou or (inter / max(area_i, 1)) > containment:
                duplicate = True
                break
        if not duplicate:
            kept.append(i)
